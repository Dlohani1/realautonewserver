...
stroke-linecap="round" st$
  ...
  $data = Lead::select('id','created_at')->where('user_id',Auth::user()->id)->whereMonth('created_at',$month)->whereYear('created_at', $year)->limit(75)->orderBy('id','asc')->get();
...
$campaigns = Campaign::where([['user_id', Auth::user()->id],['is_active',1]])->get();
        ...
        return view('ajax-list-leads',array('starterPack'=>$starterPack,'leads'=>$ids,'campaigns' => $campaigns));
    }
...
print_r($ids);die;
	...
	public function leadsRetrival(Request $request) {		
			...
			if(isset($_REQUEST['source']) && $_REQUEST['source'] == "social") {
				if ($_REQUEST['sourceVal'] == "1") {
				 ...
				 if ($_REQUEST['sourceVal'] == "3") {
					 ...
					 $whereArray[] = array('source','=',"Form HTML");
                                }
		...
		*/
		...
		if ((isset($userPack) && $userPack->pack == "1") && $filterCount > 75) {
/*
		$lead =
...
//end
		...
		$result = array("search" => $s,"draw" => $_REQUEST['draw'],"recordsTotal" => $count,"recordsFiltered" => $filterCount);
...
public function followupsAssignedRetrival() {
	...
	//$whereArray = array($where1, $where2, $where3);
       ...
       // $lead  = Lead::where($whereArray)->orWhere($orWhereFilter1)->orWhere($orWhereFilter2)->orWhere($orWhereFilter3)->orWhere($orWhereFilter4)->offset($offset)->limit($limit)->get();
        $leadFollowups = LeadFollowups::select('leads_followups.*','leads.name','leads.mobile_no','leads.project_name')
		->join('leads','leads.id','=','leads_followups.lead_id')
		->where($whereArray)
...
*/
		...
		if ($campaignName == null) {
			...
			if($leadName == null || $leadContact == null || $leadMail == null) {
				return response()->json([
		...
		if ($leadContact == null || $leadContact == "") {
			  return response()->json([
