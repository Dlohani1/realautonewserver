@extends('layouts.app')
@section('title', 'SubAdmin Leads View')
@section('content')
...
background-color: red;
            ...
            <div class="container-fluid">
                <div class="row page-title">
                    <div class="col-md-12">
                        ...
                        <h4 class="mb-1 mt-0">Assigned Lead Details</h4>
                    </div>
                </div>
  ...
  <ul class="nav nav-pills" role="tablist" style="padding:10px">
    ...
    <li class="nav-item">
      <button type="button"  class="nav-link btn btn-info" data-toggle="pill" onclick="filterData(3)">Site Visitor</button>
    </li>&nbsp;&nbsp;
        <li class="nav-item">
					...
					<div class="row">
						<div class="col-md-4 form-group">					
							...
							<select id="lead-status"  class="custom-select">
								...
								<option value="1">Hot</option>
								<option value="2">Close</option>
								<option value="3">Site Visitor</option>
								<option value="4">Fake Lead</option>
								<option value="5">Out of Location</option>
								<option value="6">Not Interested</option>
								<option value="7">Wrong No</option>
...
refreshLeads()
    ...
    function refreshLeads() {
        ...
        var t = $('#example').DataTable( {
            ...
            stateSave: true,
            destroy: true,
            ajax: "{{url('subadmin-leads-retrival')}}",
            columns: [
                {  data: null },
                {  data: null,
...
const assignedDateTime = data.created_at.split(" ");
...
var atime = da.toLocaleString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })
                ...
                {  data: null,
                    render: function(data, type, row, meta) {
                        return '<button type="button" class="btn-sm"><a href="edit-subadmin-lead/'+data.id+'"> Edit </a></button><button type="button" class="btn-sm" data-toggle="modal" data-target="#myModal2"  onclick="setleadId('+data.id+')" >Add/View </button>'
                    }
                },
...
document.getElementById("save-followup").disabled = false	
	document.getElementById("leadId").value = leadId;
		...
		function setComment() {
			...
			$.ajax({
				url : "add-subadmin-lead-comments",
				type: "POST",
				data : formData,
